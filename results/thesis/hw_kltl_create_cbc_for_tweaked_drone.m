...
if strcmp(info.Message,'Solved!')
	...
	saveas(gcf,'images/kltl-drone_turning_runs','epsc')
	saveas(gcf,'images/kltl-drone_turning_runs','png')

	%Save controller data somewhere.
	drone_controller.deconstruct_and_save_to(['data/kltl_turning_controller_data_' datestr(now,'ddmmmyyyy-HHMM') '.mat'])

else
	disp('The drone controller was not successfully created.')
end

save(['data/kltl-drone_turning_data_' datestr(now,'ddmmmyyyy-HHMM') '.mat' ])