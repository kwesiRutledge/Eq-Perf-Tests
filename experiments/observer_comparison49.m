function [results] = observer_comparison49(varargin)
	...
	% [optim6,contr6] = lk_ad.rec_synthesis( 'Equalized' , 'prefix' , 'Minimize Z2' , P_M1 , Z1 , Z1 , L );

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	%% Equalized Recovery with Zonotopes %%
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	load('data/cdc19_LK_inv_set_data.mat');

	Z2 = Zonotope(lk_cis_zono.G,lk_cis_zono.c);
	results.params.Z2 = Z2;
	...
	optim7 = optimize(positive_constr+z_incl_constr+l_diag_constr+shared_Q_constrs+shared_r_constrs+obj_constrs, ...
			obj_fcn, ...
			ops);

	optim7.M2 = value(mu2);

	results.lk_tests.new_method.opt_out = optim7;
	...
	matched_axis = [-15,15,-8,8,-15, 15];

	figure;
	ax1 = subplot(1,3,1)
	plot(Z5.to_poly().slice([1],[0]),'Color','blue')
	axis(matched_axis)
	%title(['Hypercube Template (Volume = ' num2str(Z5.to_poly().volume) ')'])

	ax2 = subplot(1,3,2)
	plot(Z3.to_poly().slice([1],[0]),'Color','blue')
	axis(matched_axis)
	%title(['Hyper Rectangle Template (Volume = ' num2str(Z3.to_poly().volume) ')'])

	ax3 = subplot(1,3,3)
	plot(Z7.to_poly().slice([1],[0]),'Color','blue')
	axis(matched_axis)
	%title(['Invariant Set Template (Volume = ' num2str(Z7.to_poly().volume) ')'])

	linkaxes([ax1,ax2,ax3],'xy')
	set(gcf,'Units','Normalized','Position',[0.1,0.1,0.75,0.5])

	saveas(gcf,'results/nahs2019/4d_lk_minM2_comparison2.eps')
end