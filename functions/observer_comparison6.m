function [ results ] = observer_comparison6( varargin )
	...
	for t_horizon = 1 : T
		...
		results.design_s0.opt_obj(t_horizon) = value(alpha0);
		value(alpha0);
		...
		results.design_s0.F{t_horizon} = value( (pinv(value(eye(size(results.design_s0.Q,1)) + results.design_s0.Q*Cm*H)) ) * results.design_s0.Q);
		results.design_s0.u0{t_horizon} = value((eye(size(results.design_s0.F,1)) + results.design_s0.F*Cm*H) * results.design_s0.r);

		%Value of s0
		results.design_s0.s0{t_horizon} = value(s0);
