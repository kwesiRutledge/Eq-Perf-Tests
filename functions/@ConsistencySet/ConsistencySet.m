classdef ConsistencySet
	...
	methods
		...
		function [cs] = ConsistencySet(varargin)
			%Description:
			...
			%
			%Example Usage:
			%	[cs] = consistent_set(lcsas,t,L)			

			%% Input Processing
			[ lcsas , KnowledgeSequence , cs_settings ] = input_processing_ConsistencySet(varargin{:});

			%% Create Internal Behavior Set
			cs.ParentInternalBehaviorSet = InternalBehaviorSet(lcsas, KnowledgeSequence);

			%% Define Object
			cs.t = length(KnowledgeSequence);
...
function [ lcsas , KnowledgeSequence , cs_settings ] = input_processing_ConsistencySet(varargin)
	%Description:
	%	Processes the inputs given to ConsistencySet() constructor.
