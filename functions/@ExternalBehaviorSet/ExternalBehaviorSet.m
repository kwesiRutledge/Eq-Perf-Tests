classdef ExternalBehaviorSet
	...
	methods
		...
		function [ebs] = ExternalBehaviorSet(varargin)
			...
			%% Input Processing
			[ lcsas , KnowledgeSequence , cs_settings ] = input_processing_ExternalBehaviorSet(varargin{:});

			%% Create Internal Behavior Set
			ebs.ParentInternalBehaviorSet = InternalBehaviorSet(lcsas, KnowledgeSequence);

			%% Get The Matrice

			%% Define Object
			ebs.t = length(KnowledgeSequence);
			ebs.System = lcsas;
			ebs.AsPolyhedron = [];
		...
		function [poly_out] = ToPolyhedron(ebs)
			...
			%% Algorithm
			poly_out = [ eye(n_x*(t+1) + n_u*t), zeros(n_x*(t+1) + n_u*t, L.cardinality()*(n_w*t + n_x) ) ] * ibs.AsPolyhedron;
